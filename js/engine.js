...
// Utilidades compartidas para sincronizar canvas/contexto y exponer
// herramientas de depuración a otros módulos (por ejemplo, al bot visual).
...
function start_main_loop(canvas, ctx) {
  ...
  ID1 = setInterval(function () {
    game_loop(resolvedCanvas, resolvedCtx);
  }, FRAME_DELAY);
...
function play_game(canvas, ctx, start_handler) {
  ...
  if (DEMO_MODE) {
    schedule_bot_action(resolvedCanvas, resolvedCtx, DEMO_INIT_BOT_DELAY, function () {
      console.log('[SYNC] Bot inicial listo, arrancando game_loop.');
      start_main_loop(resolvedCanvas, resolvedCtx);
    });
  } else {
    start_main_loop(resolvedCanvas, resolvedCtx);
  }
